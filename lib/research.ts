...
async function provider(path: string, body?: unknown) {
  if (!key()) throw new Error('Connect an OpenAI API key in the site hosting settings to search.');
  let res: Response;
  try {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401) throw new Error('The API key was rejected. Update the site’s OpenAI API key.');
...
export async function pollResearch(jobRecordId = 'research') {
  ...
  let response;
  try { response = await provider('/' + encodeURIComponent(job.responseId)); }
  catch (e) {
  ...
  if (response.status === 'queued' || response.status === 'in_progress') return researchStatus();
  ...
  catch (e) { await updateJob(job, { status: 'failed', summary: 'Search needs attention.', error: (e as Error).message }); return researchStatus(); }
  const desk = await readDesk();
