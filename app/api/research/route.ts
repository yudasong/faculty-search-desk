...
import { pollAllResearch, researchStatus, startResearch, stopTrackingResearch } from '@/lib/research';

export async function GET(request: Request) {
  const denied = await authorized(request); if (denied) return denied;
  try { return Response.json(await researchStatus(requestApiKey(request)), { headers: { 'Cache-Control': 'no-store' } }); }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json(input.action === 'start' ? await startResearch(input.scope, undefined, false, apiKey) : input.action === 'analyze' ? await startResearch('link', input.requestId, true, apiKey) : input.action === 'stop_tracking' ? await stopTrackingResearch(input.requestId, apiKey) : await pollAllResearch(apiKey), { headers: { 'Cache-Control': 'no-store' } });
  } catch (e) {
